	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau @${num.split('@')[0]} Ja foi tarde seu vagabundo 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD ALUCARD BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Alucard? Este é um recurso especial para o Alucard ❌',
					ownerB: '[❗] Alucard? Este é um recurso especial para o Alucard ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado. Registre-se para usar os comandos do bot Jack por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Jack|17\n\n──「 Jack BOT 」──`,
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		if (isGroupAdmins) return reply('vc é admin, então n irei te dar ban por usar links, rlx ademiro 🙂')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
...
break
				...
				break 
					...
					if (Number(args[0]) === 1) {
						...
						reply('Grupo anti-link ativado com sucesso neste grupo agora quem mandar link vai pra vala ✔️')
